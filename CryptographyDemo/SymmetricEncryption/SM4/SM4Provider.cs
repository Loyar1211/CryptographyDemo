...
namespace CryptographyDemo.SymmetricEncryption.SM4
{
    public class SM4Provider
    {
        ...
        public byte[] sm4_crypt_ecb(SM4Context ctx, byte[] input)
        {
            ...
            Buffer.BlockCopy(input, 0, bins, 0, length);
            byte[] bous = new byte[length];
            for (int i = 0; length > 0; length -= 16, i++)
            {
                byte[] inBytes = new byte[16];
                byte[] outBytes = new byte[16];
                Buffer.BlockCopy(bins, i * 16, inBytes, 0, length > 16 ? 16 : length);
                sm4_one_round(ctx.sk, inBytes, outBytes);
                Buffer.BlockCopy(outBytes, 0, bous, i * 16, length > 16 ? 16 : length);
        ...
        public byte[] sm4_crypt_cbc(SM4Context ctx, byte[] iv, byte[] input)
        {
            ...
            if (ctx.mode == SM4_ENCRYPT)
            {
                for (int j = 0; length > 0; length -= 16, j++)
                {
                    ...
                    Buffer.BlockCopy(out1, 0, iv, 0, 16);
                    for (int k = 0; k < 16; k++)
                    {
                        bousList.Add(out1[k]);
                    }
            ...
            else
            {
                ...
                for (int j = 0; length > 0; length -= 16, j++)
                {
                    ...
                    Buffer.BlockCopy(bins, i * 16, inBytes, 0, length > 16 ? 16 : length);
                    Buffer.BlockCopy(inBytes, 0, temp, 0, 16);
                    sm4_one_round(ctx.sk, inBytes, outBytes);
                    for (i = 0; i < 16; i++)
                    {
                        out1[i] = (byte)(outBytes[i] ^ iv[i]);
                    }
                    Buffer.BlockCopy(temp, 0, iv, 0, 16);
                    for (int k = 0; k < 16; k++)
                    {
                        bousList.Add(out1[k]);
                    }
